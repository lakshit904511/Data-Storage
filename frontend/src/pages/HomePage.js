...
const HomePage = () => {
 ...
 const deleteEmp = async ()=>{
  try{
    const deletes=await fetch(`${process.env.REACT_APP_BASE_URL}/deleteAll`,
      {
        method: "DELETE",
      }
    );
      ...
      <section className="container px-4 mx-auto py-4">
        <div className="flex items-center justify-between">
          ...
          <Link to={"/addemployee"}>
           ...
           <div>
              <button onClick={deleteEmp} className="rounded-md bg-indigo-600 px-3.5 py-1.5 text-sm font-semibold leading-7 text-white hover:bg-indigo-500 ">
                delete all Employee
              </button>
            </div> 
        </div>
